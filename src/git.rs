...
pub fn get_commit_template_path() -> Result<PathBuf> {
    ...
    if !output.status.success() {
        anyhow::bail!(
            "No template file set. Configure one using `git config --set commit.template /path/to/template/file"
        )
    }
...
pub fn get_active_buddies(buddies: &Buddies) -> Result<Buddies> {
    ...
    let mut file = File::open(&template_path).context("Failed to open commit template file")?;
...
pub fn update_commit_template(active_buddies: &Buddies) -> Result<()> {
    ...
    if !template_path.exists() {
        ...
        File::create(&template_path).context("Failed to create commit template file")?;
    }
    ...
    // Write back to file
    let mut file =
        File::create(&template_path).context("Failed to open commit template file for writing")?;
