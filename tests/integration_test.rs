...
fn setup_git_repo()
-> Result<(TempDir, String, NamedTempFile), Box<dyn std::error::Error>> {
...
fn test_create_and_list_buddy() -> Result<(), Box<dyn std::error::Error>> {
  ...
  cmd
    .args(["--buddies-file", &buddies_path, "create", "peter"])
    .current_dir(&repo_path);
  ...
  assert!(
    String::from_utf8_lossy(&output.stdout)
      .contains("Created new buddy 'peter'")
  );
...
fn test_with_and_active() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut cmd = Command::cargo_bin("git-squad")?;
  cmd
    .args(["--buddies-file", &buddies_path, "create", "wendy"])
    .current_dir(&repo_path);
  ...
  assert!(
    template_content
      .contains("Co-authored-by: Wendy Darling <wendy@darling.com>")
  );
...
fn test_without_buddy() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let mut cmd = Command::cargo_bin("git-squad")?;
  cmd
    .args(["--buddies-file", &buddies_path, "create", "hook"])
    .current_dir(&repo_path);
  ...
  assert!(
    template_content
      .contains("Co-authored-by: Captain Hook <hook@jollyroger.com>")
  );
...
fn test_alone_command() -> Result<(), Box<dyn std::error::Error>> {
  ...
  assert!(
    template_content
      .contains("Co-authored-by: Peter Pan <peter@neverland.com>")
  );
  assert!(
    template_content
      .contains("Co-authored-by: Wendy Darling <wendy@darling.com>")
